 ...
 */
function returnGivenParameter(something){
  return something;
}
 ...
 */
function takeOptionalParameter(something = 0){
...
function mapArrayToDoubleAllNumbers(arrayToDouble) {
  return arrayToDouble.map(  (number) => {  return ( 2 * number ); }   );
    // WRITE CODE HERE
}
...
function reduceArrayToFindProduct(arrayToMultiply) {
  return arrayToMultiply.reduce( 
    ...
    (num1, num2 = 1) => {  return num1*num2 ; }
    
    // Above describes the arrayToMultiply function.... it takes
    // two parameters... num1, num2... from array. If one is absent, 
    // it will assume that as 1. 
    // WRITE CODE HERE
  );
}
