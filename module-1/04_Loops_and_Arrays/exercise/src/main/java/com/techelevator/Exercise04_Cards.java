...
//        char [] cardsOfPlayer = new char [hand.length];
//
//        for (int i = 0; i < hand.length; i++) {
//            cardsOfPlayer[i] == hand[i];
//        }
//        UPDATE: NONE OF THE ABOVE IS NECESSARY...
//        Suffices to just "read" the first card of the array "hand". See below
    ...
    public String[] discardFirstCard(String[] hand) {
        ...
        String[] newHandOf4Cards = new String[hand.length-1];

        newHandOf4Cards[0] = hand[1];
        newHandOf4Cards[1] = hand[2];
        newHandOf4Cards[2] = hand[3];
        newHandOf4Cards[3] = hand[4];   //declared all 4 objects in the String "newHandOf4Cards"


        return newHandOf4Cards;
    }
    ...
    discardTopCard([]) → []
     */
    public String[] discardTopCard(String[] originalDeck) {

        if (originalDeck.length >= 2) {

            String[] new_Deck = new String[originalDeck.length - 1];

            for (int i = 0; i < originalDeck.length - 1; i++) {

                new_Deck[i] = originalDeck[i + 1];
            }
