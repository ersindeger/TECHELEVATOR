...
public class Exercise01_StoreOrders {
    ...
    public int[] createOrder() {
        ...
        int [] orders = new int[] {SMALL_CHEESE, CALZONE , LARGE_PEPPERONI, SPAGHETTI_PIE};
    ...
    public int getCheesePizzaRevenue(int[] orders) {
        ...
        for (int i = 0; i < orders.length; i++) {
            if ( orders[i] == SMALL_PEPPERONI || orders[i] == SMALL_CHEESE ){
                cost = cost + 8;
            }
            else if (orders[i] == MEDIUM_CHEESE || orders[i] == MEDIUM_PEPPERONI) {
                cost = cost + 11;
            }
            else if (orders[i] == LARGE_CHEESE || orders[i] == LARGE_PEPPERONI) {
                cost = cost + 14;
            }
