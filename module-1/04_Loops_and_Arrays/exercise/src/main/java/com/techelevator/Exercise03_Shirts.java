...
public class Exercise03_Shirts {
    ...
    public char[] buildOrder() {
        ...
        char [] orderOfCustomer = new char[] { SMALL_TSHIRT, 'S', 'S', 'M', 'M', 'L'};
            // notice SMALL_TSHIRT and 'S' are interchangeable... so are the other ones done similarly.
    ...
    public char[] buildBulkOrder(int numberOfShirts) {
        ...
        char [] customerOrderList = new char [numberOfShirts];

        for (int i = 0; i < numberOfShirts ; i++) {
            int j = i % 3;
            if (j == 0){
                customerOrderList[i] = SMALL_TSHIRT;
            }
            else if (j == 1 ){
                customerOrderList[i] = MEDIUM_TSHIRT;
            }
            else{
                customerOrderList[i] = LARGE_TSHIRT;
            }
    ...
    public boolean placeRequest(char[] order) {
        ...
        boolean orderToBePlaced = false;

        for (int i = 0; i < order.length; i++) {
            if(order[i] == SMALL_TSHIRT){
                orderToBePlaced = true;
            }
        }
