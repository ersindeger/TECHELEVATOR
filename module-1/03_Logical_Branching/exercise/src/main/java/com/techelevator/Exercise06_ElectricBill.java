...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed) {
        ...
        else if (unitsUsed <= BASIC_SERVICE_LIMIT && unitsUsed > 0){
    ...
    public double calculateElectricBill(double unitsUsed, boolean hasRenewableEnergy) {
        if (hasRenewableEnergy){
            ...
            if(unitsUsed > BASIC_SERVICE_LIMIT){
                return (1-RENEWABLE_ENERGY_DISCOUNT) * (( (unitsUsed - BASIC_SERVICE_LIMIT) * EXCESS_SERVICE_RATE ) + ( BASIC_SERVICE_LIMIT * BASIC_SERVICE_RATE ));
            }

            else if (unitsUsed <= BASIC_SERVICE_LIMIT && unitsUsed > 0){
                return (1-RENEWABLE_ENERGY_DISCOUNT) * ( unitsUsed * BASIC_SERVICE_RATE );
            }
    ...
    public double calculateElectricBill(double unitsUsed, double unitsReturned) {
        ...
        if(netUsage <= 100) {
            ...
            if (unitsReturned == 0 || netUsage < 0) {
                ...
                return netUsage * (1 - RENEWABLE_ENERGY_DISCOUNT) * BASIC_SERVICE_RATE;
            }
            ...
            if ( unitsReturned == 0 ) {
                ...
                return (( (unitsUsed - BASIC_SERVICE_LIMIT) * BASIC_SERVICE_RATE ) + (BASIC_SERVICE_LIMIT * BASIC_SERVICE_RATE)) * (1 - RENEWABLE_ENERGY_DISCOUNT);
            }
