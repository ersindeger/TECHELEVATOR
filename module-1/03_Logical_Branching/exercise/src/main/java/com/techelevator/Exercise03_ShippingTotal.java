...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        if (weightPounds <= MAX_WEIGHT_POUNDS && weightPounds > 0)
            ...
            return MAX_WEIGHT_POUNDS*UP_TO_40_LB_RATE + OVER_40_LB_RATE * (weightPounds-MAX_WEIGHT_POUNDS);
        else
            return 0;
                                                            }
    ...
    if (hasDiscount){
        if (weightPounds <= MAX_WEIGHT_POUNDS && weightPounds > 0)
            ...
            return (MAX_WEIGHT_POUNDS*UP_TO_40_LB_RATE + OVER_40_LB_RATE * (weightPounds-MAX_WEIGHT_POUNDS)) * 0.90;
        else
            return 0;}
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS && weightPounds > 0)
            ...
            return (MAX_WEIGHT_POUNDS*UP_TO_40_LB_RATE + OVER_40_LB_RATE * (weightPounds-MAX_WEIGHT_POUNDS))*(1-discountPercentage);
        else
            return 0;
